...
/*
Dwarf Generator - Labyrinth Lord
...
char* primeReqBonus(unsigned strength);
...
int main(void)
{
	/*Holds the dwarf's name*/
	char dwarfName[ARRAY_SIZE];

	/*Holds the dwarf's alignment*/
	...
	/*Saving throws*/
	unsigned breathAttacks = 13;
	unsigned poisonDeath = 8;
	unsigned petrifyParalyze = 10;
	unsigned wands = 9;
	unsigned spells = 12;
	...
	/*Stores dwarf class bonuses*/
	char dwarfBonuses[ARRAY_SIZE] = "\nAbility to see in the dark (infravision) up to 60'.\n2 in 6 chance of detecting traps,false walls, hidden\nconstruction or noticing a sloped passage.";

	/*Save file name*/
	char saveFile[ARRAY_SIZE] = "";

	/*Introduction text*/
	printf_s("Welcome to the Labyrinth Lord Dwarf Generators.\n", ARRAY_SIZE);

	/*prompt user to enter dwarf's name*/
	printf_s("\nEnter the name of your Dwarf: \n", ARRAY_SIZE);

	scanf_s("%[^\n]s", dwarfName, ARRAY_SIZE);

	/*displays the name user entered*/
	printf_s("\nThe name you entered is: %s\n", dwarfName, ARRAY_SIZE);
	...
	do {
		...
		/*if statement used to ensure constitution is minimum of 9 as dwarf
		class requirement*/
		if (constitution < 9)
		{
			constitution = 9;
		...
		strcpy(primeRequisiteBonus, primeReqBonus(strength));
		...
		printf_s("\nNotes: %s%s%s", primeRequisiteBonus, charismaReaction, dwarfBonuses, ARRAY_SIZE);
	...
	printf_s("\nYou have %u gold pieces (gp) to spend on your dwarf.  Now it is time to buy some armour, weapons and gear.\n\n", gold, ARRAY_SIZE);
	...
	/*Option of purchasing a shield if dwarf has at least 10 gp*/
	if (gold >= 10)
	{
	...
	printf_s("\nNow, let's purchase some weapons for your dwarf.\n", ARRAY_SIZE);

	/*Function for adding weapons for dwarf*/
	do
	{
		choice = 0;
		...
		printf_s("1. Longsword        10 gp     1d8\n", ARRAY_SIZE);
		printf_s("2. Hand Axe         1 gp      1d6\n", ARRAY_SIZE);
		printf_s("3. Mace             5 gp      1d6\n", ARRAY_SIZE);
		printf_s("4. Dagger           3 gp      1d4\n", ARRAY_SIZE);
		printf_s("5. Silver Dagger   25 gp      1d4\n", ARRAY_SIZE);
		printf_s("6. Javelin          1 gp      1d6\n", ARRAY_SIZE);
		printf_s("7. Crossbow (heavy) 25 gp     1d8\n", ARRAY_SIZE);
		printf_s("8. Crossbow (light) 16 gp     1d6\n", ARRAY_SIZE);
		printf_s("9. Spear             6 gp     1d6\n", ARRAY_SIZE);
		printf_s("10. Shortbow         25 gp    1d6\n", ARRAY_SIZE);
		printf_s("11. No Weapons\n", ARRAY_SIZE);
		...
		case 1:
			if (gold > 10)
			{
				expense = 10;
				gold -= expense;
				strcat(weapons, "Longsword");
				printf_s("\nYou have purchased a Longsword\n", ARRAY_SIZE);
			}
			else
			{
				printf_s("\nYou do not have enough gold for the Longsword\n", ARRAY_SIZE);
			}
		...
		case 2:
			if (gold > 1)
			{
				expense = 1;
				gold -= expense;
				strcat(weapons, "Hand Axe");
				printf_s("\nYou have purchased a Hand Axe\n", ARRAY_SIZE);
			}
			else
			{
				printf_s("\nYou do not have enough gold for the Hand Axe\n", ARRAY_SIZE);
			}
		...
		case 6:
			if (gold > 1)
			{
				expense = 1;
				gold -= expense;
				strcat(weapons, "Javelin");
				printf_s("\nYou have purchased a Javelin\n", ARRAY_SIZE);
			}
			else
			{
				printf_s("\nYou do not have enough gold for the Javelin\n", ARRAY_SIZE);
			}
		...
		case 7:
			if (gold > 25)
			{
				expense = 25;
				gold -= expense;
				strcat(weapons, "Crossbow (heavy)");
				printf_s("\nYou have purchased a Crossbow (heavy)\n", ARRAY_SIZE);
			}
			else
			{
				printf_s("\nYou do not have enough gold for the Crossbow (heavy)\n", ARRAY_SIZE);
			}
		...
		case 8:
			if (gold > 16)
			{
				expense = 16;
				gold -= expense;
				strcat(weapons, "Crossbow (light)");
				printf_s("\nYou have purchased a Crossbow (light)\n", ARRAY_SIZE);
			}
			else
			{
				printf_s("\nYou do not have enough gold for the Crossbow (light)\n", ARRAY_SIZE);
			}
		...
		case 9:
			if (gold > 6)
			{
				expense = 6;
				gold -= expense;
				strcat(weapons, "Spear");
				printf_s("\nYou have purchased a Spear\n", ARRAY_SIZE);
			}
			else
			{
				printf_s("\nYou do not have enough gold for the Spear\n", ARRAY_SIZE);
			}
		...
		case 10:
			if (gold > 25)
			{
				expense = 25;
				gold -= expense;
				strcat(weapons, "Shortbow");
				printf_s("\nYou have purchased a Longbow\n", ARRAY_SIZE);
			}
			else
			{
				printf_s("\nYou do not have enough gold for the Longbow\n", ARRAY_SIZE);
			}
	...
	/*Function for adding gear for dwarf*/
	do
	{
		choice = 0;
	...
	printf_s("\nHere is an update of your dwarf:\n", ARRAY_SIZE);
	...
	printf_s("\nDo you want to save your dwarf (%s) to a text files?\n1.yes\n2.no\n(please select 1 (yes) or 2 (no)\n", dwarfName, ARRAY_SIZE);

	scanf(" %d", &noYes);

	if (noYes == 1)
	{
		printf_s("\nPlease a name (no spaces, please) for the file you want to save the dwarf's information to: ", ARRAY_SIZE);
...
/*Randomly generates the dwarf's hit points
...
/*displays the prime requisite bonus for dwarf*/
char* primeReqBonus(unsigned strength)
{
